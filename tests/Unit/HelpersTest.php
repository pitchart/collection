...
class HelpersTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testMapReturnsAMappedCollection() {
		$collection = ch\map([1,2,3,4], function ($item) { return $item + 1; });
		self::assertInstanceOf(Collection::class, $collection);
	...
	{
		self::assertEquals([2, 3, 4, 5], ch\map($iterable, function ($item) { return $item + 1; })->values());
	}

	public function testFilterReturnsAFilteredCollection() {
		$collection = ch\filter([1,2,3,4], function ($item) { return $item % 2 == 0; });
		self::assertInstanceOf(Collection::class, $collection);
	...
	{
		self::assertEquals([2, 4], ch\filter($iterable, function ($item) { return $item % 2 == 0; })->values());
	}

	public function testRejectReturnsAFilteredCollection() {
		$collection = ch\reject([1,2,3,4], function ($item) { return $item % 2 == 0; });
		self::assertInstanceOf(Collection::class, $collection);
	...
	{
		self::assertEquals([1, 3], ch\reject($iterable, function ($item) { return $item % 2 == 0; })->values());
	}

	public function testReduceReturnsAReducedValue() {
		$reduced = ch\reduce([1,2,3,4], function ($accumulator, $item) { return $accumulator + $item; }, 0);
	...
	{
		self::assertEquals(10, ch\reduce($iterable, function ($accumulator, $item) { return $accumulator + $item; }, 0));
	}

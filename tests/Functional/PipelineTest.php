...
class PipelineTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testIntegerArrayPipeline() {
        ...
        $pipelineReductionSum = $collection->filter(function($item) { return $item % 2  == 0; })
            ->map(function($item) { return $item * 2; })
            ->reduce(function($accumulator, $item) { return $accumulator + $item; }, 0);
    ...
    public function testObjectArrayPipelines() {
        ...
        $emailList = $collection
            ->reject(function(Character $character) { return (boolean) $character->getEmail() == null; })
            ->map(function(Character $character) { return $character->getEmail(); })
        ;
        $this->assertContains('jay@quick.stop', $emailList);
        $this->assertEquals(7, $emailList->count());

        $peopleAtQuickStop = $collection
            ->filter(function(Character $character) { return preg_match('/@quick.stop$/', $character->getEmail());})
            ->map(function (Character $character) {return $character->getAge();})
        ;
        $averageAgeAtQuickStop = $peopleAtQuickStop->reduce(function($accumulator, $age) {
            return $accumulator + $age;
        }, 0) / $peopleAtQuickStop->count();
